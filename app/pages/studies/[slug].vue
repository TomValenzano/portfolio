...
<template>
  <article class="mx-auto max-w-3xl px-6 py-16 md:py-24">
    <header class="hero-stagger space-y-5 mb-14">
      ...
      >
        <span aria-hidden="true" class="inline-block transition-transform duration-200 group-hover:-translate-x-0.5">←</span> {{ t('project.back_to_studies') }}
      </NuxtLink>
      ...
      <dl class="grid grid-cols-2 sm:grid-cols-3 gap-y-3 gap-x-6 pt-4 border-t border-[color:var(--color-border)]">
        <div v-if="project.role">
          <dt class="font-mono text-[10px] uppercase tracking-wider text-[color:var(--color-subtle)]">{{ t('project.course') }}</dt>
