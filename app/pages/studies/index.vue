  ...
  <section class="mx-auto max-w-5xl px-6 py-16 md:py-24">
    ...
    <div v-if="projects?.length" class="grid gap-4">
      <NuxtLink
        ...
        class="group block p-6 rounded-lg border border-[color:var(--color-border)] bg-[color:var(--color-surface)]/40 hover:bg-[color:var(--color-surface)] hover:border-[color:var(--color-accent)] transition-colors"
      >
        <div class="flex items-start justify-between gap-6 mb-2">
          <h2 class="text-xl md:text-2xl font-semibold tracking-tight group-hover:text-[color:var(--color-accent)] transition-colors">
            {{ project.title }}
          </h2>
