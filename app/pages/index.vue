...
<template>
  <section class="min-h-[calc(100dvh-3.5rem)] flex items-center px-6">
    <div class="hero-stagger mx-auto w-full max-w-4xl space-y-8 py-24">
      <p class="font-mono text-sm text-[color:var(--color-accent)] flex items-center gap-2">
        <span class="terminal-cursor font-semibold" aria-hidden="true">▎</span>
        {{ t('hero.kicker') }}
      </p>
      ...
      <div class="flex flex-wrap gap-3 pt-4">
        <NuxtLink
          :to="localePath('/work')"
          class="group inline-flex items-center gap-2 px-5 py-3 rounded-md bg-[color:var(--color-accent)] text-[color:var(--color-bg)] font-medium hover:bg-[color:var(--color-accent-dim)] transition-colors"
        >
          {{ t('hero.cta_work') }}
  ...
  <section v-if="featured?.length" v-reveal class="mx-auto max-w-4xl px-6 pb-24">
    <div class="flex items-baseline justify-between mb-8">
      <h2 class="font-mono text-sm text-[color:var(--color-accent)]">// {{ t('home.featured') }}</h2>
      <NuxtLink
        :to="localePath('/work')"
        class="group font-mono text-xs text-[color:var(--color-muted)] hover:text-[color:var(--color-accent)] transition-colors"
      >
        {{ t('home.all_work') }} <span class="inline-block transition-transform duration-200 group-hover:translate-x-0.5">→</span>
      </NuxtLink>
    </div>
